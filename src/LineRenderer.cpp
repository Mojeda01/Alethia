...
std::vector<std::uint32_t> readFileWords(const std::filesystem::path& path) {
    ...
    if (size <= 0 || (size % 4) != 0) {
        throw std::runtime_error("Invalid shader file: " + path.string());
    }
...
LineRenderer::LineRenderer(VkDevice dev, VkRenderPass rp, VkDescriptorSetLayout dsLayout)
    : device(dev), renderPass(rp)
{
    if (device == VK_NULL_HANDLE || renderPass == VK_NULL_HANDLE || dsLayout == VK_NULL_HANDLE) {
        throw std::invalid_argument("LineRenderer: invalid Vulkan handles");
    }
    ...
    try {
        const auto vertWords = readFileWords(shaderPath("line.vert.spv"));
        const auto fragWords = readFileWords(shaderPath("line.frag.spv"));
        ...
        if (vkCreatePipelineLayout(device, &pl, nullptr, &tmpLayout) != VK_SUCCESS) {
            throw std::runtime_error("LineRenderer: vkCreatePipelineLayout failed");
        }
        ...
        auto bindingDesc = LineVertex::bindingDescription();
        auto attribDescs = LineVertex::attributeDescriptions();
...
VkShaderModule LineRenderer::createShaderModule(const void* bytes, size_t sizeBytes) const {
    ...
    if (vkCreateShaderModule(device, &ci, nullptr, &mod) != VK_SUCCESS) {
        throw std::runtime_error("LineRenderer: vkCreateShaderModule failed");
    }
    return mod;
}
